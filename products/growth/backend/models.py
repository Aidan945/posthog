...
class EnrichmentSignupSnapshot(UUIDModel):
    ...
    row is purely the idempotency guard and provenance timestamp. organization_id has no foreign
    key so the marker never takes a lock on the hot organization table, and its unique constraint
