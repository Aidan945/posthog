...
/// Enqueue an analytics event on the global client. Non-blocking: since
/// posthog-rs 0.14 the SDK owns batching, retries, and delivery on its own
/// background worker, so there is nothing to spawn or await here and delivery
/// failures surface through the `on_error` hook wired in `common_posthog::init`.
pub fn fire_capture(event: Event) {
