...
async def resolve_quota_status(request: Request, team_id: int | None, resource_key: str) -> QuotaResourceStatus:
    ...
    # Mirror extract_token's precedence: x-api-key wins over Authorization.
    # Auth accepts either header, so forwarding only Authorization would let an
    # over-quota caller bypass the check by sending their token via x-api-key.
